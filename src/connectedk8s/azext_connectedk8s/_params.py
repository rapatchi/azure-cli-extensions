...
def load_arguments(self, _):
    ...
    with self.argument_context('connectedk8s get-credentials') as c:
        ...
        c.argument('path', options_list=['--file', '-f'], type=file_type, completer=FilesCompleter(), default=os.environ.get('KUBECONFIG', os.path.join(os.path.expanduser('~'), '.kube', 'config')), help="Kubernetes configuration file to update. If not provided, defaulting to, if KUBECONFIG is set, updates that file, else '~/.kube/config'. Use '-' to print YAML to stdout instead.")
